...
def _readme(ep, meta, rep, script, canon_counts):
    ...
    n_anim = rep.get("animated_shots", 0)
    anim_line = ("" if not n_anim else
                 f"\n* **Animated shots:** {n_anim} of {len(rep['cuts'])}. Location plates only -- "
                 f"characters are heraldic cards and are never animated, and diagrams are "
                 f"information displays where motion costs legibility. Each clip's motion is "
                 f"prompted from the narration of the segment it appears under.")
...
* **Audio:** {loud}. Target -14 LUFS / -1 dBTP. No background music, by request.{anim_line}
