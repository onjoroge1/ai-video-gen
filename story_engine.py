...
def cadence_block(fmt: StoryFormat, answer_terms=None) -> str:
    ...
    block = (
        ...
        'that reads as competent and forgettable. Reach for the middle only when neither register '
        'fits.\n'
        '  · WHICH REGISTER IS NOT YOUR CHOICE — IT IS SET BY THE ROLE YOU ARE WRITING. This is a '
        'per-scene requirement, and it is the one to check before you return a scene:\n'
        '      - EXPLAINING ROLES — mechanism, reversal, consequence, false_belief, '
        'second_revelation, scope_shift, credential — MUST contain exactly one sentence of 15 WORDS '
        'OR MORE, in one unbroken span, carrying that beat\'s causal step. Count the words in that '
        'sentence before you return the scene. A scene in one of these roles that contains no '
        '15-word sentence is wrong even if it reads well.\n'
        '      - TURN ROLES — anomaly, seal, resolution, personal_bridge — lead with the short '
        'punch; five words or fewer, landing the reveal or the stake.\n'
        '    Stated per scene because the percentage above is not something you can measure while '
        'writing one scene, and every attempt to hit it as a percentage has come back at 5-10% '
        'long. You can, however, check one sentence in the scene in front of you. Splitting the '
        'long sentence into two shorter ones does not satisfy this — the causal step has to arrive '
        'in a single span, because that is the thing a voice cannot deliver any other way.\n'
