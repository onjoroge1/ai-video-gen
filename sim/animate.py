    ...
    def one(sc):
        ...
        if os.path.exists(out) and os.path.getsize(out) > 10000:
            ...
            if ok.returncode == 0 and float(ok.stdout.strip() or 0) > 1.0:
                with lock:
                    manifest[sc.id] = out
                RL.log(sim.slug, "clip", sc.id, _t0, True, "reuse (probed)", 0.0, "cache")
        ...
        with lock:
            if spent + price_per_clip > cap_usd + 1e-6:
                ...
                RL.log(sim.slug, "clip", sc.id, _t0, False, "spend cap reached", 0.0, "")
        ...
        if not (ok and os.path.exists(out) and os.path.getsize(out) > 10000):
            ...
            RL.log(sim.slug, "clip", sc.id, _t0, False, str(err)[:160], 0.0, model)
            ...
            if not inv["passed"]:
                ...
                with lock:
                    failures.append((sc.id, f"invented subject in empty scene "
                                            f"(blob {inv['score']})"))
                RL.log(sim.slug, "clip", sc.id, _t0, False,
                       f"invented subject blob {inv['score']}", price_per_clip, model)
        ...
        if not accept(m, min_motion):
            ...
            RL.log(sim.slug, "clip", sc.id, _t0, False,
                   f"near-static mean {m['per_frame_mean']} local {m['local_frac']}",
                   price_per_clip, model)
            progress(f"  {sc.id:16s} REJECT mean {m['per_frame_mean']} local {m['local_frac']}")
            os.replace(out, out + ".rejected")
            return
        with lock:
            manifest[sc.id] = out
        RL.log(sim.slug, "clip", sc.id, _t0, True,
               f"mean {m['per_frame_mean']} local {m['local_frac']}", price_per_clip, model)
