...
def render_scene(plate_path, out_mp4, frames, fps, px_per_m, d_m, v_ms,
    ...
    # The splash lasts a fixed 0.9s of REAL time, so under slow motion it stretches with everything
    # else. It used to be 0.9s of PLAYBACK regardless of slow_mo, which meant the fall ran at 1/4
    # speed and then the crown snapped back to full speed -- the two halves of one event playing at
    # two different rates.
    splash_frames = int(fps * 0.9 * max(slow_mo, 1))
    ...
    for f in range(frames):
        ...
        for ei, (f0, x) in enumerate(events):
            ...
            if df <= n_fall:
                ...
                t = (df - n_fall) / fps            # PLAYBACK seconds; real seconds are t/slow_mo
                ...
                # Time dilation is not one division. With t in playback seconds the real elapsed
                # time is t/s, so a velocity term picks up 1/s and an ACCELERATION term picks up
                # 1/s^2. Gravity was divided by s once, which made the crown's apex shrink by
                # exactly 1/s -- at 1/4 speed the splash threw a quarter as high, while the time to
                # reach that apex did not change at all. That is the precise inverse of what slow
                # motion means: it must change WHEN, never HOW FAR.
                g_slow = g_px / (max(slow_mo, 1) ** 2)
                for vx, vy, size in crowns[ei]:
                    px = x + vx * v_px * t
                    py = y_impact - (vy * v_px * t - 0.5 * g_slow * t * t)
                    if py < y_impact + 4:
